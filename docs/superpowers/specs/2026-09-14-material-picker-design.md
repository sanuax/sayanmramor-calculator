...
- Клик по остальной части карточки (включая карточки "нет в наличии") —
  `onSelect(stoneId)`, закрытие модалки. Существующий `calculate()` уже
  умеет показывать "нужно уточнить у менеджера" для камня без слэбов —
  дополнительной логики в калькуляторе не требуется.
